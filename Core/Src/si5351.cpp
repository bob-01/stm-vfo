 ...
 *
 */
bool Si5351::init(I2C_HandleTypeDef *i2c, uint8_t xtal_load_c, uint32_t xo_freq, int32_t corr)
{
  _i2c = i2c;
...
{
  HAL_StatusTypeDef status = HAL_I2C_Mem_Write(_i2c, i2c_bus_addr << 1, addr, I2C_MEMADD_SIZE_8BIT, data, sizeof(uint8_t)*bytes, 500);
  return status == HAL_OK;
}

uint8_t Si5351::si5351_write(uint8_t addr, uint8_t data)
{
  HAL_StatusTypeDef status = HAL_I2C_Mem_Write(_i2c, i2c_bus_addr << 1, addr, I2C_MEMADD_SIZE_8BIT, &data, sizeof data, 500);
  return status == HAL_OK;
}
...
uint8_t Si5351::si5351_read(uint8_t addr)
{
  ...
  HAL_I2C_Master_Transmit(_i2c, i2c_bus_addr << 1, &addr, 1, 100);

  if (HAL_I2C_Master_Receive(_i2c, i2c_bus_addr << 1, &reg_val, sizeof(uint8_t), 100) == HAL_OK) {
    return reg_val;
  }
